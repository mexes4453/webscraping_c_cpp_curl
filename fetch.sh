...
do
    ...
    # edit query
    QUERY_STATS_VAL_CUR=$(echo ${QUERY_STATS_VAL} | sed -n "s/COIN/${SYM}/gp")
    QUERY_HISTORY_CUR=$(echo ${QUERY_HISTORY} | sed -n "s/COIN/${SYM}/gp")

    # show url query
    echo ${QUERY_STATS_VAL_CUR}

    # fetch data 
    curl -L ${QUERY_STATS_VAL_CUR} | jq > "${SYM}_VAL.${FILE_OUT_EXT}"
    #curl -L ${QUERY_HISTORY_CUR} | jq > "${SYM}_HIST.${FILE_OUT_EXT}"
